...
class Custom:
    ...
    def LowCmdWrite(self):
        if self.firstRun:
            for i in range(12):
                self.startPos[i] = self.low_state.motor_state[i].q  # 记录当前站立状态
            self.firstRun = False
        ...
        for i in range(12):
            self.low_cmd.motor_cmd[i].q = (1 - self.percent_4) * self.startPos[i] + self.percent_4 * self._targetPos_3[i]
        ...
        if self.percent_4 == 1.0:
            print("Robot has successfully laid down!")
            time.sleep(1)  # 让机器人稳定
            sys.exit(0)  # 退出程序
